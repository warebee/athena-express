...
module.exports = class AthenaExpress {
    constructor(init) {
        ...
        this.config = {
            athenaClient: init.athenaClient,
            s3Client: init.s3Client,
            s3Bucket: init.s3 ||
                `s3://athena-express-${init.s3Client.config.credentials().accessKeyId
                    .substring(0, 10)
    ...
    async query(query, values, options) {
        ...
        if (typeof query === "object") {
            ...
            if (loweredCaseKeys.hasOwnProperty("parameters")) {
                config.parameters = loweredCaseKeys.parameters;
            }

            if (query.hasOwnProperty("includeMetadata")) {
                config.includeMetadata = query.includeMetadata;
            }
        } else if (
        ...
        try {
            ...
            const queryStatus = await helpers.checkIfExecutionCompleted(config, context);
                ...
                if (/.txt/.test(s3Output) || /.csv/.test(s3Output)) {
                    try {
                        ...
                        if (config.includeMetadata) {
                            results.Metadata = queryResultsFromS3.metadata;
                        }
