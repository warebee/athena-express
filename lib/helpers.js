    ...
    const keepCheckingRecursively = async function () {
      try {
        ...
        if (data.QueryExecution.Status.State === "SUCCEEDED") {
          retry = config.retry;
          s3Metadata = config.athenaClient.send(new GetQueryResultsCommand({
              QueryExecutionId: config.QueryExecutionId,
              MaxResults: 1,
            }));
          resolve(data);
...
async function getQueryResultsFromS3(params) {
  ...
  if (params.statementType === "UTILITY" || params.statementType === "DDL") {
    ...
    const metadataQueryPromise = params.config.includeMetadata ? await params.config.athenaClient.send(new GetQueryResultsCommand({
      QueryExecutionId: params.config.QueryExecutionId,
      MaxResults: 1
    })) : null;
