"use strict";
const { StartQueryExecutionCommand, GetQueryExecutionCommand, GetQueryResultsCommand, StopQueryExecutionCommand } = require("@aws-sdk/client-athena");
    ...
    const keepCheckingRecursively = async function () {
      try {
        if (config.timeout && Date.now() - startTime >= config.timeout) {
          config.athenaClient
            .send(new StopQueryExecutionCommand({ QueryExecutionId: config.QueryExecutionId }))
            .catch(() => {});
          reject(new Error(`Athena query timed out after ${config.timeout}ms`));
          return;
        }
...
async function cleanUpDML(input, ignoreEmpty, context) {
  let cleanJson = [];
  const dataTypes = await getDataTypes(context);
  const columnNames = (await context.s3Metadata).ResultSet.ResultSetMetadata
    .ColumnInfo.map((columnInfo) => columnInfo.Name);
  return new Promise(function (resolve) {
    input.pipe(
      csv({
        ignoreEmpty,
        // Athena writes NULL cells as empty CSV fields, so an all-NULL row of
        // a single-column result is an entirely EMPTY line. Parse to raw cell
        // arrays and map them onto the metadata column names ourselves:
        // csvtojson's default json output drops value-less rows entirely and
        // (with ignoreEmpty) omits empty cells from the row object, whereas
        // every row must come back with every column present - `null` where
        // the cell is blank (addDataType maps empty strings to null).
        // `ignoreEmpty` still controls whether entirely empty lines are
        // skipped (true) or returned as all-NULL rows (false).
        output: "csv",
      })
        .on("data", (data) => {
          const cells = JSON.parse(data.toString("utf8"));
          const row = {};
          for (let i = 0; i < columnNames.length; i++) {
            row[columnNames[i]] = cells[i] === undefined ? "" : cells[i];
          }
          cleanJson.push(addDataType(row, dataTypes));
        })
        .on("finish", function () {
          resolve(cleanJson);
        })
